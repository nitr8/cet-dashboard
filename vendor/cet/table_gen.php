<?php 
	function generateDayOffBoxFromResponse($response)
	{
		if($response->isError()) 
		{
			...
			{
				$dayoffArray[ strtotime($value->start).$i]= array(date("D jS F", strtotime($value->start)), 
				date("D jS F", strtotime($value->end)),
				$value->amount,
				$value->type,
				$value->employee,                         
			...
			foreach ($dayoffArray as $value)
			{
				...
				if (time() >= $value[5]) 
				{
					$returnvalue .= "<td width=\"24px\"><img src=\"vendor/cet/img/onleave.png\" />";
				}
				$returnvalue .="</tr>";
			}
			$returnvalue .= "</table>";				
